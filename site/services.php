...
<div style="background:var(--c-light);border-bottom:1px solid rgba(var(--c-primary-rgb),.08);position:sticky;top:66px;z-index:150">
  <div class="container">
    <div style="display:flex;gap:0;overflow-x:auto;scrollbar-width:none">
      ...
      <a href="#svc-<?= $svc['id'] ?>" style="padding:14px 20px;font-size:.88rem;font-weight:700;color:var(--c-text);white-space:nowrap;border-bottom:3px solid transparent;transition:all .2s;text-decoration:none;display:inline-flex;align-items:center;gap:6px"
         onmouseover="this.style.color='var(--c-primary)';this.style.borderBottomColor='var(--c-primary)'"
...
<section id="svc-<?= $svc['id'] ?>" style="padding:72px 0;<?= $idx%2===1?'background:var(--c-light)':'' ?>">
  <div class="container">
    <div style="display:grid;grid-template-columns:<?= $idx%2===1?'1.4fr 1fr':'1fr 1.4fr' ?>;gap:60px;align-items:center;margin-bottom:48px">
      ...
      <div class="animate-in">
        <div style="display:flex;align-items:flex-start;gap:14px;margin-bottom:20px">
          ...
          <div>
            <h2 style="font-size:clamp(1.4rem,3vw,1.8rem);font-weight:900;color:var(--c-primary);line-height:1.2"><?= h($svc['name']) ?></h2>
            <?php if($svc['price_text']): ?><div style="display:inline-block;margin-top:6px;background:rgba(var(--c-accent-rgb),.15);color:var(--c-accent);font-weight:800;font-size:.9rem;padding:4px 14px;border-radius:20px"><?= h($svc['price_text']) ?></div><?php endif; ?>
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:24px">
          ...
          foreach($features as $f): ?>
            <div style="font-size:.85rem;font-weight:600;color:var(--c-primary);padding:8px 12px;background:rgba(var(--c-primary-rgb),.06);border-radius:8px"><?= $f ?></div>
          <?php endforeach; ?>
        </div>
      ...
      <div class="animate-in delay-2">
        ...
        <?php else: ?>
          <div style="font-size:6rem;text-align:center;padding:60px 0;background:var(--c-light);border-radius:20px"><?php $fi=$svc['icon']??''; if($fi && str_ends_with($fi,'.svg')): ?><img src="<?= BASE_URL.'/'.h($fi) ?>" style="width:120px;height:120px;margin:0 auto" alt=""><?php else: echo h($fi?:'🛠️'); endif; ?></div>
        <?php endif; ?>
      </div>
    ...
    <div class="animate-in" style="max-width:760px;margin:0 auto">
      <h3 style="font-size:1.1rem;font-weight:800;color:var(--c-primary);margin-bottom:16px;padding-bottom:10px;border-bottom:2px solid rgba(var(--c-primary-rgb),.12)">❓ 常見問題</h3>
      <div style="display:flex;flex-direction:column;gap:8px">
        <?php foreach($svc['faqs'] as $fi=>$faq): ?>
        <div style="border:1.5px solid rgba(var(--c-primary-rgb),.1);border-radius:10px;overflow:hidden;background:#fff">
          <button onclick="toggleFaq(this)" aria-expanded="false"
            style="width:100%;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:16px 18px;background:none;border:none;cursor:pointer;text-align:left;font-size:.92rem;font-weight:700;color:var(--c-text);transition:all .2s"
            onmouseover="this.style.background='rgba(var(--c-primary-rgb),.04)'"
            onmouseout="this.style.background=''">
            <span><?= h($faq['question']) ?></span>
            <svg style="width:18px;height:18px;flex-shrink:0;color:var(--c-primary);transition:transform .25s" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M6 9l6 6 6-6"/></svg>
          </button>
